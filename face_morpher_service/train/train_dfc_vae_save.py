...
flags = tf.app.flags
flags.DEFINE_integer("epoch", 500, "Epoch to train [5]")
...
flags.DEFINE_integer("batch_size", 48, "The number of batch images [64]")
...
flags.DEFINE_integer("sample_step", 500, "The interval of generating sample. [300]")
...
def main(_):
    ...
    # Prepare computational graph that uses GPU
    with tf.device("/gpu:0"):
        ##========================= DEFINE MODEL ===========================##
        # the input_imgs are input for both encoder and discriminator
        input_imgs = tf.placeholder(tf.float32,[FLAGS.batch_size, FLAGS.output_size, FLAGS.output_size, FLAGS.c_dim], name='real_images')
    ...
    for epoch in range(FLAGS.epoch):
        ...
        if True:
            # --------- Sample per epoch image ----------
            # generate and visualize generated images
            img1, img2 = sess.run([gen2.outputs, gen3.outputs], feed_dict={input_imgs: original_set_of_images})
            save_images(img1, [8, 8],
                        './{}/for_anim/train_{:02d}.png'.format(samples_1, epoch))

            # save input image for comparison
            save_images(original_set_of_images,[8, 8],'./{}/for_anim/input.png'.format(samples_1))
            print("[Sample] for_anim generated!!!")
            # --------- Sample per epoch image ended ----------
    ...
    # Persist whole graph that would be restored in prod state
    saver = tf.train.Saver()
    save_path = saver.save(sess, os.path.join(save_dir, 'weights.ckpt'))
